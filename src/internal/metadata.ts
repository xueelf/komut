...
const COMMAND_OPTIONS = Symbol('komut.command-options');
const OPTION_REGISTRY = Symbol('komut.option-registry');
const PROGRAM_OPTIONS = Symbol('komut.program-options');
...
type KomutMetadata = DecoratorMetadataObject & {
  [COMMAND_OPTIONS]?: CommandOptions;
  [OPTION_REGISTRY]?: Map<string, RegisteredOption>;
  [PROGRAM_OPTIONS]?: ProgramOptions;
};

const metadataByClass = new WeakMap<object, KomutMetadata>();

function requireDecoratorMetadata(metadata: DecoratorMetadata): KomutMetadata {
  if (!metadata) {
    throw new TypeError('Komut requires Bun decorator metadata support.');
  }
  return metadata;
}

function getOwnMetadata(target: object): KomutMetadata | undefined {
  return metadataByClass.get(target);
}

function bindClassMetadata(
  context: ClassDecoratorContext,
  metadata: KomutMetadata,
): void {
...
export function bindOptionMetadata<This, Value>(
  ...
  const komutMetadata = requireDecoratorMetadata(context.metadata);

  context.addInitializer(function () {
    metadataByClass.set(this as object, komutMetadata);
  });
}
...
export function setProgramOptions(
  ...
  komutMetadata[PROGRAM_OPTIONS] = options;
  bindClassMetadata(context, komutMetadata);
}
...
export function setCommandOptions(
  ...
  komutMetadata[COMMAND_OPTIONS] = options;
  bindClassMetadata(context, komutMetadata);
}
...
export function registerOption(
  ...
  const komutMetadata = requireDecoratorMetadata(metadata);
  const registeredOptions = Object.hasOwn(komutMetadata, OPTION_REGISTRY)
    ? komutMetadata[OPTION_REGISTRY]
    : undefined;
  const options = registeredOptions ?? new Map<string, RegisteredOption>();

  if (!registeredOptions) {
    komutMetadata[OPTION_REGISTRY] = options;
  }
