...
async function runInputInTerminal(
  ...
  return runPromptInTerminal('input.ts', color, (terminal, output) => {
    if (!inputSent && output.includes('(komut):')) {
      inputSent = true;
...
async function runValidatedInputInTerminal(
  ...
  return runPromptInTerminal(
    ...
    color,
    (terminal, output) => {
      if (!invalidInputSent && output.includes('(komut):')) {
        invalidInputSent = true;
        terminal.write('komut!\r');
      } else if (
        !correctionSent &&
...
describe('input prompt', () => {
  ...
  test('validates submitted values without an interactive terminal', async () => {
    ...
    expect(valid.result).toBe('komut');
    expect(defaulted.exitCode).toBe(0);
    expect(defaulted.result).toBe('komut');
  ...
  test('uses defaults only for submitted empty input', async () => {
    ...
    expect(empty.result).toBe('komut');
  ...
  test('preserves whitespace entered by the user', async () => {
    ...
    expect(result.result).toBe('  Komut  ');
  });
  ...
  if (process.platform !== 'win32') {
    ...
    test('matches Bun input output with and without color', async () => {
      ...
      expect(plain.rawOutput).toBe(
        'package name (komut): audit\r\nRESULT=audit\r\n',
      );
      expect(colored.rawOutput).toBe(
        '\x1b[0m\x1b[36mpackage name\x1b[0m \x1b[2m(komut):\x1b[0m audit\r\nRESULT=audit\r\n',
      );
    });
    ...
    test('redraws validation errors without repeating the prompt', async () => {
      ...
      expect(plain.result).toBe('komut');
      ...
      expect(plain.rawOutput).toBe(
        'package name (komut): ' +
          ...
          'komut!\r\n' +
          '\x1b8\x1b[0Jkomut!\r\n' +
          '> Only lowercase letters are allowed.\x1b8komut!' +
          '\x1b8\x1b[0Jkomut' +
          '\x1b8\x1b[0Jkomut\r\n' +
          'RAW=false\r\n' +
          'RESULT=komut\r\n',
      ...
      expect(colored.result).toBe('komut');
