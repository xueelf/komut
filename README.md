...
Komut is a decorator-based CLI framework developed on top of Bun. You can easily create your own command-line tools using the latest ECMAScript decorator syntax.
...
```typescript
import { Command, execute, Option, Program } from 'komut';
...
Here is a complete single-file example showing how to quickly build a command-line tool with arguments and options using Komut:
...
After saving the `cli.ts` code, you can run it directly with Bun. Komut will automatically collect the defined decorator metadata upon application startup, and finally output the CLI panel:
...
Komut deeply replicates the default behavior of Bun's native CLI, aiming to bring a consistent and harmonious experience to developers using Bun.
...
`@Command` is used to declare an individual executable command. When this command is invoked, Komut will parse the arguments input in the terminal and inject them sequentially into the instance object of that class:
...
It's not constrained to solely CLI decorators alone. Komut implements terminal interactions with Bun and TypeScript, without shipping C sources or platform-specific binaries. Apart from that, text beautification tools are provided.
...
```typescript
import { colorize } from 'komut';

// Single color
console.log(colorize('cyan', 'Hello, Komut!'));
...
```typescript
import { select } from 'komut/prompts';
...
The message and displayed choice labels must each fit on one logical line. When the list is taller than the terminal, Komut keeps the selected choice inside a terminal-sized visible window.
